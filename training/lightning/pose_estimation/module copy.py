...
class PoseEstimationModule(pl.LightningModule):
    def __init__(
        ...
        print("\n=== Initializing PoseEstimationModule ===")
        print("This message should appear when the module is created")
        """
        ...
        print("\n=== Setting up model in PoseEstimationModule ===")
        self.model = model
        print("Model type:", type(self.model))
        print("Setting task to pose_estimation...")
    ...
    def forward(self, x):
        ...
        if isinstance(output, VitPoseEstimatorOutput):
            print(f"Output heatmaps shape: {output.heatmaps.shape}")
            print(f"Heatmap stats - Min: {output.heatmaps.min():.3f}, Max: {output.heatmaps.max():.3f}, Mean: {output.heatmaps.mean():.3f}")
        else:
    ...
    def _generate_target_heatmap(self, keypoints, visibility, size):
        ...
        """
        print("\n=== Target Heatmap Generation Details ===")
        print(f"Input shapes:")
        print(f"- keypoints: {keypoints.shape}")
        print(f"- visibility: {visibility.shape}")
        print(f"- target size: {size}")
        print(f"\nKeypoint stats:")
        print(f"- x coords: min={keypoints[..., 0].min():.3f}, max={keypoints[..., 0].max():.3f}")
        print(f"- y coords: min={keypoints[..., 1].min():.3f}, max={keypoints[..., 1].max():.3f}")
        print(f"- visibility values: {torch.unique(visibility).tolist()}")
        ...
        keypoints[..., 0] = keypoints[..., 0] / 640  # Original image width
        keypoints[..., 1] = keypoints[..., 1] / 640  # Original image height
        # Then scale to heatmap size and apply UDP offset
        keypoints[..., 0] = keypoints[..., 0] * W - 0.5
        keypoints[..., 1] = keypoints[..., 1] * H - 0.5
        
        print("\nNormalized keypoint stats:")
        print(f"X range: [{keypoints[..., 0].min():.3f}, {keypoints[..., 0].max():.3f}]")
        print(f"Y range: [{keypoints[..., 1].min():.3f}, {keypoints[..., 1].max():.3f}]")
        ...
        for b in range(B):
            for n in range(N):
                ...
                if kpts_visible.any():
                    ...
                    print(f"\nPerson {n} in batch {b}:")
                    print(f"- Visible keypoints: {kpts_visible.sum().item()}/{K}")
                    print(f"- Bounding box: ({xmin:.1f}, {ymin:.1f}) to ({xmax:.1f}, {ymax:.1f})")
                    print(f"- Person scale: {person_scale:.1f}")
                    print(f"- Adaptive sigma: {adaptive_sigma:.3f}")
                else:
                    adaptive_sigma = base_sigma
                    print(f"\nPerson {n} in batch {b}: No visible keypoints")
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(f"- Images stats - Min: {images.min():.3f}, Max: {images.max():.3f}, Mean: {images.mean():.3f}")
        print(f"- Keypoints shape: {keypoints.shape}")
        print(f"- Masks shape: {masks.shape}")
        print(f"- Valid instances in batch: {masks.sum().item()}")
        print(f"- Keypoints visibility stats:")
        ...
        print("\n=== Processing Valid Instances ===")
        print(f"Total instances per batch item: {masks.shape[1]}")
        print(f"Number of valid instances per batch item: {masks.sum(dim=1).tolist()}")
        
        # Create lists to store valid instances
        valid_coords_list = []
        valid_visibility_list = []
        
        # Process each batch item
        for b in range(len(masks)):
            # Get valid instances for this batch item
            valid_mask = masks[b]  # [N]
            if valid_mask.sum() == 0:
                print(f"Warning: Batch item {b} has no valid instances!")
                continue
                
            # Extract valid instances
            valid_coords = keypoints[b, valid_mask, :, :2]  # [valid_N, K, 2]
            valid_visibility = keypoints[b, valid_mask, :, 2]  # [valid_N, K]
            
            valid_coords_list.append(valid_coords)
            valid_visibility_list.append(valid_visibility)
        
        if not valid_coords_list:
            print("No valid instances found in entire batch!")
            return None
        
        # Stack valid instances
        coords = torch.cat(valid_coords_list, dim=0)  # [total_valid_N, K, 2]
        visibility = torch.cat(valid_visibility_list, dim=0)  # [total_valid_N, K]
        
        print(f"After filtering - Valid instances: {len(coords)}")
        ...
        target_heatmaps, target_weights = self._generate_target_heatmap(
            coords.unsqueeze(0),  # Add batch dimension [1, N, K, 2]
            visibility.unsqueeze(0),  # Add batch dimension [1, N, K]
            self.heatmap_size
        )
        
        print("\n=== Target Heatmaps Generated ===")
        print(f"Target heatmaps shape: {target_heatmaps.shape}")
        print(f"Target heatmap stats - Min: {target_heatmaps.min():.3f}, Max: {target_heatmaps.max():.3f}, Mean: {target_heatmaps.mean():.3f}")
        print(f"Target weights shape: {target_weights.shape}")
        print(f"Target weights stats - Min: {target_weights.min():.3f}, Max: {target_weights.max():.3f}, Mean: {target_weights.mean():.3f}")
        
        if target_heatmaps is None:
            print("Skipping batch due to no valid instances")
            return None
            
        print("\n=== Forward Pass in Training Step ===")
        print("Model type:", type(self.model))
        print("Model's current task:", getattr(self.model, 'current_task', 'Unknown'))
        print("About to call model's forward pass...")
        ...
        print(f"Predicted heatmap stats - Min: {pred_heatmaps.min():.3f}, Max: {pred_heatmaps.max():.3f}, Mean: {pred_heatmaps.mean():.3f}")
        
        # Reshape predictions to match our single-batch target format
        B, K, H, W = pred_heatmaps.shape
        pred_heatmaps = pred_heatmaps.mean(dim=0, keepdim=True)  # Average predictions across batch
        print(f"Averaged predictions shape: {pred_heatmaps.shape}")
        
        # Apply online hard example mining (OHEM)
        pred_reshaped = pred_heatmaps.reshape(1, K, -1)  # [1, K, H*W]
        target_reshaped = target_heatmaps.reshape(1, K, -1)  # [1, K, H*W]
        ...
        if torch.isnan(total_loss) or torch.isinf(total_loss):
            ...
            print(f"Smoothness loss is NaN: {torch.isnan(smoothness_loss).any()}")
            print(f"Smoothness loss is inf: {torch.isinf(smoothness_loss).any()}")
